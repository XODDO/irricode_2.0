...
String Kit_ID = "Hoima IrriKit"; // --- "MakRIF II IrriKit 002", "MakRIF II IrriKit 001";
...
const uint8_t tank_level_sensorPin[2] = {6, 8}; // --- Display.object.TankMessage[0]; 
const uint8_t tank_signal[3] = {10, 11, 12}; // --- to show different levels: empty, half and full
const uint8_t pumpPin = 47;
const uint8_t connected_soil_sensors[4] = {A5, A4, A3, A2};
const uint8_t soil_moisture_signal[4] = {A10, A11, A12, A13};
const uint8_t humi_temp_signal = A9;
const uint8_t humidity = A6;

const uint8_t valvePin = 48; //46, 48, 49;
...
const uint8_t power = 39,  back = 43, set = 45;
const uint8_t up = 37, down = 35, left = 41, Okay = 33, right = 22; 
const uint8_t dim = 26, full_beam = 24;
//const uint8_t dim = 28, full_beam = 26;
const uint8_t power_indica = A8, danger = A15;
...
uint8_t second, minute, hour, day=1, month=1, Y1=20, Y2=21; 
uint16_t year = 0;
String zero = "0";
...
void Setting() {
  ...
  LCD.setFont(u8g_font_helvR08);LCD.setPrintPos(3, 51);   LCD.print("3. Factory Reset"); 
...
void notification_popup(String notification, int x1 = 0, int y1 = 0, int button_count=1){
// Head
// Body
...
LCD.setColorIndex(0);LCD.drawRBox(18, 8, 92, 48,4);LCD.setColorIndex(1);LCD.drawRFrame(18, 8, 92, 48, 4); 
LCD.setFont(u8g_font_helvB08);   LCD.setPrintPos(27, 26);   LCD.print(notification); 
...
//---title
    LCD.setFont(u8g_font_tpssb);  LCD.setPrintPos(36, 21);   LCD.print("irri-kit UI "); //LCD.print(scroll);
...
//send to several numbers
              ...
              //String phoneNumber[2] = {"AT+CMGS=\"+256759991993\"", "AT+CMGS=\"+256757330484\""};
                    ...
                    SIM.println("AT+CMGS=\"+256759991993\"");   //  --- Recipient number (s) --- MasterLine 
                      updateSerial();
                      SIM.print("Today on: "); SIM.print(Kit_ID); SIM.println(); updateSerial();
              
              SIM.print(all); updateSerial();  
                /*
...
public: bool already_saved = false; bool already_sent = false;
...
String irrigation_duration = irri_Parameter[3] + " in minutes";
...
String field_humidity = irri_Parameter[6] + "%";
   ...
   String binded_4_transmission[8][2] = {{"Data Round: ", String(message_counter)},
                                         {"Soil Moisture before irrigation: ", moisture_before_irrigation},
                                         {"Soil Moisture just after irrigation:  ", moisture_level_now},
                                         {"Irrigation started at:  ", irrigation_start_time},
                                         {"And lasted for:  ", irrigation_duration},
                                         {"With water amounting to: ", irrigation_volume},
...
notification_ = already_sent?"Sent!":"Failed!";
  ...
  void send_Message(String Irrigation_Message){
    SIM.println("AT"); //Initiate data transmission when a certain number of handwashs has been bundled up
                    ...
                    String sendTo = "";
                    String numberDB[4] = {"AT+CMGS=\"+256759991993\"", "AT+CMGS=\"+256755279145\"", "AT+CMGS=\"+256701853322\"", "AT+CMGS=\"+256751712004\""};
...
while(loader<100){ loader++; pos++; flip = !flip; //................................
    ...
    if(loader>=0 && loader<=40){ 
      ...
      else if(loader == 7){pinMode(full_beam, OUTPUT);  pinMode(dim, OUTPUT);    digitalWrite(full_beam, 1);}
      else if(loader == 8){
...
LCD.firstPage();  
 ...
 // LCD.setPrintPos(62, 1);  LCD.print(F("Initializing Sensors!"));
     ...
     if(loader == 40){digitalWrite(soil_moisture_signal[0], LOW);}
...
for(int i=0; i<4; i++){ digitalWrite(soil_moisture_signal[i], LOW);}
  digitalWrite(humi_temp_signal, LOW);
...
time_slot = EEPROM.read(1000);

    day = EEPROM.read(time_slot+3);
if(day != 0) set_up = true; // if day is not zero ...  a day is never zero
   ...
   if(set_up) {
    ...
    Y1 = EEPROM.read(time_slot+5);
    Y2 = EEPROM.read(time_slot+6);

    current_Screen = 9;
  tone(sound_fx, 400); delay(50); tone(sound_fx, 2100, 100); 
    }     
} 
// setup()
...
// --- TIME STORAGE ---- //
String Month = "Unset"; // --- 7 Time Parameters --- 
...
unsigned long track_time(); bool ManagePower(uint8_t * battery_reader);
...
//---heavily event-driven program
  ...
  while(!set_up){  system_setup();  }
    ...
    current_millis  =  track_time(); 
    //schedulable schedulable tasks first
        tank_unlocked = false;
    if(second%30 == 0) tank_unlocked=true;
           
    if(hour == 18) { send_unlocked = true;}
    else {send_unlocked = false; data.already_sent = false; }

    if(hour%3 == 0 && minute == 0 && second == 0) save_unlocked = true;
...
// the time constraint thing --- how do we know it would be irrigating when time is not right???
         ...
         if(moisture_now > 13.50) {/* Do Nothing, if ON, power OFF */ irrigationPrompt = "Soil Moisture OK";
              if(valve_is_ON) {digitalWrite(valvePin, HIGH); valve_is_ON=false;} }
    else if(moisture_now <= 11.50){
...
//       data.Save(); 
      ...
      if(send_unlocked == true){
         if(data.already_sent==false) {
              uint8_t hh_mm_ss[3] = {hour, minute, second};
  ...
  m=0;  active_moisture_sensors = 0;
    // --- The following is based on the assumption that we know them...
     //  m1 = analogRead(A4);  m2 = analogRead(A7);  m3 = analogRead(A6);  m4 = analogRead(A5);
       ...
       m_1 = analogRead(connecteddy[0]);  delay(10);
       m_2 = analogRead(connecteddy[1]);  delay(10);
       m_3 = analogRead(connecteddy[2]);  delay(10);
       m_4 = analogRead(connecteddy[3]);  delay(10);
       
      
    if(m_1 <= 5 || m_2 <= 5 || m_3 <= 5 || m_4 <= 5){  M_Readings[0] = M_Readings[1] = M_Readings[2] = M_Readings[3] = "Power!!";   }
else{
     
  if(m_1 >= 1020){ M_Readings[0] = "Sensor Out"; plugged_1 = false; last_state1 = false;}
  else { plugged_1 = true; 
    ...
    //y = 0.0211x + 3.7722;
    m1 = 0.021*m1 + 3.80;
    m1 = map(m1, 0.00, 25.00, 20.00, 3.00);
  
      /*m1 = constrain(m1,0,5);*/ 
      M_Readings[0] = String(int(m1)+0.50);// + "V";
      m+=m1; active_moisture_sensors++; } 
  
  if(m_2 >= 1020){ M_Readings[1] = "Sensor Out";  plugged_2 = false; last_state2 = false;}
  else { plugged_2 = true; 
    if(plugged_2 && !last_state2){digitalWrite(lights[2], 1); delay(50); /*tone(sound_fx, 2000,50);*/   digitalWrite(lights[2], 0); last_state2 = true;}
        m2 = float(m_2); //m2 = constrain(m2,0,5); 
        m2 = 0.021*m2 + 3.80;
        m2 = map(m2, 0.00, 25.00, 20.00, 3.00);
        M_Readings[1] = String(int(m2)+0.50);// + "V"; 
        m+=m2; active_moisture_sensors++; }

        if(m3 != 0.00) M_Readings[2] = String(int(m3)+0.50); else M_Readings[2] = "Out";
        if(m4 != 0.00) M_Readings[3] = String(int(m4)+0.50); else M_Readings[3] = "Out";
        

    /*
  if(m_3 >= 1020){ M_Readings[2] = "Sensor Out";  plugged_3 = false; last_state3 = false;}
    else {plugged_3 = true; 
    if(plugged_3 && !last_state3){digitalWrite(lights[1], 1);delay(50);    digitalWrite(lights[1], 0); last_state3 = true;}
        m3 = float(map(m_3, 1023,0, 0,1023)); //m3 = constrain(m3,0,5); 
        m3 = 0.0211*m3 + 3.7722;
        M_Readings[2] = String(int(m3)+0.50);// + "V";
        m+=m3; active_moisture_sensors++;   }

    
  if(m_4 >= 1020){ M_Readings[3] = "Sensor Out";  plugged_4 = false; last_state4 = false;}
    else { plugged_4 = true;  
    if(plugged_4 && !last_state4){digitalWrite(lights[0], 1); delay(50);    digitalWrite(lights[0], 0); last_state4 = true;}
        m4 = float(map(m_4, 1023,0, 0,1023)); //m4 = constrain(m4,0,5); 
        m4 = 0.0211*m4 + 3.7722;
        M_Readings[3] = String((m4)+0.50);// + "V"; 
       m+=m4; active_moisture_sensors++;   } */
}
    /*
    if(m1 - m4 > 7.50) {// ignore m4
...
byte _btn; 
     ...
     if(digitalRead(set))  _btn = set; else if(digitalRead(up)) _btn = up;
else if(digitalRead(back)) _btn = back; else if(digitalRead(Okay)) _btn = Okay;
else if(digitalRead(left)) _btn = left; else if(digitalRead(down)) _btn = down;
else if(digitalRead(right)) _btn = right;else if(digitalRead(power)) {delay(500); _btn = power;}
...
byte soil_tank_field[4] = {average_soil_moisture, (byte)water_percent, (byte)temp, (byte)humi};
  ...
  //   = {String(EEPROM.read(2000)), String(EEPROM.read(2500)), String(EEPROM.read(3000))};
      String Farm_Details[]   = {"Hoima", "1 acre", "veggiez", "18/2/22"};
...
//cast_card_details();
                          ...
                          String Farmer_Details[] = {"Byaruhanger", "Hoima", "Veggiez", "Irrigation"}; //{String(EEPROM.read(1000)), String(EEPROM.read(1500)), String(EEPROM.read(2000))};
...
*/
  int menu_select(int selected){
    ...
    else if(selected == 4) {if(!valve_is_ON) {digitalWrite(valvePin, LOW); valve_is_ON=true;} else {digitalWrite(valvePin, LOW); valve_is_ON=false;} jump_to = 10;}
...
if(current_Screen == 0){  // --- settings screen
  switch(_btn){
            ...
            case back: case power: tone(sound_fx, 123, 30); current_Screen = 8;  break;
  ...
  save = true;
        ...
        switch(_btn){
            ...
            case back: case power: current_Screen = SleepNow; tone(sound_fx, 123, 30); break;
        ...
        switch(_btn){
            case set:  case Okay: current_Screen = (Screen.set_option==1?25:21); 
                        if(Screen.set_option==1){Screen.temp_hr = hour;  Screen.temp_min = minute;} 
                        else data.send_settings(); break; // if backlit_state == false{digitalWrite(back_light, HIGH);}
            case back:  current_Screen = 10;  tone(sound_fx, 123, 30); break;
